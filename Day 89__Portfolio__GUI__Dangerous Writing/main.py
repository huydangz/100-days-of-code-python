...
root.config(padx=20, pady=20, bg=BACKGROUND)


def check_text():
    global count, current_text
    if len(text_entry.get()) > 0 and text_entry.get() == current_text:
        message_label.config(text=f"Disappearing in: {count}")
        if count == 0:
            text_entry.delete(0, END)
            message_label.config(text=f"Now start again...")
...
title_label = Label(text="Dangerous Writing App", font=(TITLE_FONT, 54, "bold"), fg=TITLE, bg=BACKGROUND)
